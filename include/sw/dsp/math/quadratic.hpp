...
template <DspField T>
std::complex<T> solve_quadratic_1(T a, T b, T c) {
...
template <DspField T>
std::complex<T> solve_quadratic_2(T a, T b, T c) {
	using complex_t = std::complex<T>;
	return (complex_t(-b) - std::sqrt(complex_t(b * b - T{4} * a * c))) / (T{2} * a);
}
...
template <DspField T>
std::pair<std::complex<T>, std::complex<T>> solve_quadratic(T a, T b, T c) {
