...
// ============================================================================
// Frequency test patterns
// ============================================================================

// Zone plate: concentric sinusoidal rings with increasing spatial frequency.
// The frequency increases linearly from center to edge, sweeping across the
// spatial bandwidth. This is the canonical test pattern for evaluating the
// spatial frequency response of image processing operators.
//
// f(r,c) = 0.5 + 0.5 * cos(k * (dx^2 + dy^2))
// where k controls the maximum frequency at the corners.
template <DspField T>
	requires ConvertibleToDouble<T>
mtl::mat::dense2D<T> zone_plate(std::size_t rows, std::size_t cols,
                                 double max_freq = 0.0) {
	mtl::mat::dense2D<T> img(rows, cols);
	double cy = static_cast<double>(rows - 1) * 0.5;
	double cx = static_cast<double>(cols - 1) * 0.5;
	...
	for (std::size_t r = 0; r < rows; ++r) {
		for (std::size_t c = 0; c < cols; ++c) {
			...
			img(r, c) = static_cast<T>(0.5 + 0.5 * std::cos(max_freq * r2));
		}
	}
	return img;
}
...
// Binary threshold: output is high where input >= threshold, low otherwise.
template <DspOrderedField T>
mtl::mat::dense2D<T> threshold(const mtl::mat::dense2D<T>& image, T thresh,
                                T low = T{}, T high = static_cast<T>(1)) {
	...
	for (std::size_t r = 0; r < rows; ++r)
		for (std::size_t c = 0; c < cols; ++c)
			result(r, c) = (image(r, c) >= thresh) ? high : low;
	return result;
}

// Add noise to an image (clamps result to [0, 1]).
template <DspField T>
	requires ConvertibleToDouble<T>
mtl::mat::dense2D<T> add_noise(const mtl::mat::dense2D<T>& image,
	...
	for (std::size_t r = 0; r < rows; ++r) {
		for (std::size_t c = 0; c < cols; ++c) {
			double v = static_cast<double>(image(r, c)) + dist(gen);
			if (v < 0.0) v = 0.0;
			if (v > 1.0) v = 1.0;
			result(r, c) = static_cast<T>(v);
		}
	}
	return result;
}
