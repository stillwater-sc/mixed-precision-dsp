...
void test_butterworth_lowpass_response() {
	...
	if (!(cascade.num_stages() == 2)) throw std::runtime_error("test failed: cascade.num_stages() == 2");  // 4th order = 2 biquad stages

	// At DC (f=0), response should be ~1.0 (0 dB)
	auto r_dc = cascade.response(0.0);
	double mag_dc = std::abs(r_dc);
	if (!(near(mag_dc, 1.0, 0.01))) throw std::runtime_error("test failed: near(mag_dc, 1.0, 0.01)");
	...
	if (!(near(db_cutoff, -3.0, 0.5))) throw std::runtime_error("test failed: near(db_cutoff, -3.0, 0.5)");  // within 0.5 dB of -3 dB
	...
	if (!(db_high < -40.0)) throw std::runtime_error("test failed: db_high < -40.0");  // 4th order = -80 dB/decade, at ~1 decade above
...
void test_butterworth_highpass_response() {
	...
	double mag_high = std::abs(r_high);
	if (!(near(mag_high, 1.0, 0.1))) throw std::runtime_error("test failed: near(mag_high, 1.0, 0.1)");

	// At cutoff, response should be ~-3 dB
	double fc = 1000.0 / 44100.0;
	auto r_cutoff = cascade.response(fc);
	double db_cutoff = 20.0 * std::log10(std::abs(r_cutoff));
	if (!(near(db_cutoff, -3.0, 0.5))) throw std::runtime_error("test failed: near(db_cutoff, -3.0, 0.5)");
	...
	if (!(db_low < -40.0)) throw std::runtime_error("test failed: db_low < -40.0");
...
void test_butterworth_bandpass_response() {
	...
	if (!(db_center > -6.0)) throw std::runtime_error("test failed: db_center > -6.0");  // should be near 0 dB
	...
	if (!(db_dc < -10.0)) throw std::runtime_error("test failed: db_dc < -10.0");
...
void test_butterworth_bandstop_response() {
	...
	if (!(db_center < -10.0)) throw std::runtime_error("test failed: db_center < -10.0");

	// At DC, should pass through near 0 dB
	auto r_dc = cascade.response(0.0);
	double mag_dc = std::abs(r_dc);
	if (!(near(mag_dc, 1.0, 0.2))) throw std::runtime_error("test failed: near(mag_dc, 1.0, 0.2)");
...
void test_butterworth_impulse_response() {
	...
	for (int n = 0; n < N; ++n) {
		...
		if (!(std::isfinite(h_df2[n]))) throw std::runtime_error("test failed: std::isfinite(h_df2[n])");
	}
	...
	for (int n = 0; n < N; ++n) {
		if (!(near(h_df2[n], h_df1[n], 1e-10))) throw std::runtime_error("test failed: near(h_df2[n], h_df1[n], 1e-10)");
		if (!(near(h_df2[n], h_tdf2[n], 1e-10))) throw std::runtime_error("test failed: near(h_df2[n], h_tdf2[n], 1e-10)");
	}
	...
	if (!(energy_tail < energy_head)) throw std::runtime_error("test failed: energy_tail < energy_head");
...
void test_butterworth_simple_filter() {
	...
	if (!(std::isfinite(y2))) throw std::runtime_error("test failed: std::isfinite(y2)");
	if (!(y0 != 0.0)) throw std::runtime_error("test failed: y0 != 0.0");
...
void test_butterworth_odd_order() {
	...
	const auto& cascade = filter.cascade();
	if (!(cascade.num_stages() == 2)) throw std::runtime_error("test failed: cascade.num_stages() == 2");  // 3rd order: 1 pair + 1 single = 2 stages

	auto r_dc = cascade.response(0.0);
	if (!(near(std::abs(r_dc), 1.0, 0.01))) throw std::runtime_error("test failed: near(std::abs(r_dc), 1.0, 0.01)");

	// Process some samples
	std::array<DirectFormII<double>, 2> state{};
	double y = cascade.process(1.0, state);
	if (!(std::isfinite(y))) throw std::runtime_error("test failed: std::isfinite(y)");
...
void test_butterworth_orders() {
	...
	for (int order = 1; order <= 8; ++order) {
		...
		if (!(near(std::abs(r_dc), 1.0, 0.02))) throw std::runtime_error("test failed: near(std::abs(r_dc), 1.0, 0.02)");

		double fc = 5000.0 / 48000.0;
		auto r_cutoff = filter.cascade().response(fc);
		double db = 20.0 * std::log10(std::abs(r_cutoff));
		if (!(near(db, -3.0, 1.0))) throw std::runtime_error("test failed: near(db, -3.0, 1.0)");  // -3dB at cutoff within 1dB
	}
