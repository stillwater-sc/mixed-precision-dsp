...
// ---------------------------------------------------------------------------
// Elliptic: stub throws.
// ---------------------------------------------------------------------------
static void test_elliptic_stub() {
	bool threw = false;
	try { elliptic_prototype(4, 1000.0, 0.5, 40.0); }
	catch (const std::exception&) { threw = true; }
	if (!threw)
		throw std::runtime_error("elliptic_prototype: expected stub throw");
}
...
int main() {
	try {
		...
		test_elliptic_stub();                    std::cout << "  elliptic_stub_throws     PASS\n";
